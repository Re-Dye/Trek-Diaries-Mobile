...
export const preferences = pgTable("preferences", {
	user_id: text("user_id").primaryKey().notNull().references(() => users.id, { onDelete: "cascade" } ),
...
export const users_to_locations = pgTable("users_to_locations", {
	user_id: text("user_id").notNull().references(() => users.id, { onDelete: "cascade" } ),
	location_id: uuid("location_id").notNull().references(() => locations.id, { onDelete: "cascade" } ),
},
(table) => {
	return {
		...
		users_to_locations_user_id_location_id: primaryKey({ columns: [table.user_id, table.location_id], name: "users_to_locations_user_id_location_id"}),
	}
});

export const users_like_posts = pgTable("users_like_posts", {
	user_id: text("user_id").notNull().references(() => users.id, { onDelete: "cascade" } ),
	post_id: uuid("post_id").notNull().references(() => posts.id, { onDelete: "cascade" } ),
},
(table) => {
	return {
		users_like_posts_user_id_post_id: primaryKey({ columns: [table.user_id, table.post_id], name: "users_like_posts_user_id_post_id"}),
	}
});